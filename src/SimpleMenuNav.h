...
void SMN_handler(Button2 &btn)
{
  ...
  case SINGLE_CLICK:
    if (btn.getAttachPin() == BUTTON_UP)
    {
      ...
      if (c > 0)
        active_menu->setSelectedChild(--c);
    }
    else
    {
      int c = active_menu->getSelectedChild();
      ...
      else
      {
        if (clicked_menu->actionSelect != NULL || clicked_menu->actionSelectWithParam != NULL)
        {
          ...
          if (clicked_menu->actionSelect != NULL)
            clicked_menu->actionSelect();
          if (clicked_menu->actionSelectWithParam != NULL)
            clicked_menu->actionSelectWithParam(clicked_menu->param);
          if (clicked_menu->alertDone)
          {
            if (String(clicked_menu->name).length() > 6)
            {
              SMN_alert("done!", 500, 0);
            }
            else
            {
              SMN_alert(String(clicked_menu->name) + " done!", 500, 0);
            }
          }
          lastClick = millis();
        }
...
void SMN_printMenu()
{
  ...
  tft.setTextColor(fileStatus[pcurrent] ? TFT_RED : TFT_WHITE, TFT_BLACK);
  String p = "[";
  p += pcurrent;
  p += "]";
  SMN_printAt(p, 180, 2);
  ...
  {
    String entry = "";
    if (active_menu->getSelectedChild() == i)
      entry += "(*)";
    else
      entry += "( )";
