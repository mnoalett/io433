...
void custom_signal()
{ // This saves the custom code into the current page's code
  tft.fillScreen(TFT_BLACK);
  tft.drawRect(0, 0, WIDTH - 1, HEIGHT - 1, TFT_WHITE);
  tft.setFreeFont(FMB24);
  tft.setTextColor(TFT_RED, TFT_WHITE);
  ...
  tft.drawString(String(CUSTOMBUFSIZE), 35, 45, GFXFF); // Displays the number of transitions in the custom code
  delay(100);
}
...
void frequencyAnalyzer()
{
  ...
  int xPos = (tft.width() - tft.textWidth(text)) / 2;
  SMN_printAt(text, xPos, 15);
  tft.setFreeFont(FMB18);
  text = "000.000 MHz";
  xPos = (tft.width() - tft.textWidth(text)) / 2;
  tft.drawString(text, xPos, 50, GFXFF);
  ...
  {
    tft.fillScreen(TFT_BLACK);
    text = String(markFreq) + " MHz";
    xPos = (tft.width() - tft.textWidth(text)) / 2;
    tft.drawString(text, xPos, 50, GFXFF);
    tft.setFreeFont(FMB12);
    tft.drawString("RSSI: " + String(markRssi) + " dBm", 10, HEIGHT - 20, GFXFF);
...
void bruteforce(int protocolIndex)
{
  ...
  int xPos = (tft.width() - tft.textWidth(protocol.name)) / 2;
  tft.drawString(protocol.name, xPos, 10, GFXFF);
  ...
  for (int i = 0; i <= keyset; i++)
  {
    ...
    tft.setFreeFont(FMB18);
    String completed = String(i) + "/" + String(keyset);
    xPos = (tft.width() - tft.textWidth(completed)) / 2;
    tft.drawString(completed, xPos, 45, GFXFF);
...
void sendPagerSignal()
{
  ...
  for (int key = 0; key <= keyset; key++)
  {
    String composedValue = "11062" + String(key);
    send(composedValue.toInt(), 24);
    if (SMN_isAnyButtonPressed())
    {
      return;
    }
...
void setup()
{
  ...
  SimpleMenu *menu_scanner = new SimpleMenu("Freq.Analyzer", menu_main, frequencyAnalyzer);
  ...
  SimpleMenu *menu_monitor = new SimpleMenu("Dump", menu_more, dump);
  ...
  SimpleMenu *menu_customsig = new SimpleMenu("Load", menu_extra, custom_signal);

  SimpleMenu *more_protocols;
  SimpleMenu *menu_protocols;
  int totalItems = sizeof(protocols) / sizeof(protocols[0]);
  for (int i = 0; i < totalItems; i++)
  {
    if (i < 3)
    {
      menu_protocols = new SimpleMenu(protocols[i].name, menu_bruteforce, bruteforce, i);
    }
    if (i == 3)
    {
      more_protocols = new SimpleMenu("More", menu_bruteforce, NULL);
      new SimpleMenu(protocols[i].name, more_protocols, bruteforce, i);
    }
    if (i > 3)
    {
      new SimpleMenu(protocols[i].name, more_protocols, bruteforce, i);
  ...
  for (int f = 0; f < MAXSIGS; f++)
  {
    ...
    fileStatus[f] = loadSPIFFS(fname.c_str(), signal433_store[f], BUFSIZE);
  }
